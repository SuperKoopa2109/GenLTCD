...
def get_img_num_per_cls(dataset, cls_num, imb_factor):
...
def subsample_test_dataset(dataset):
    img_num_per_cls = get_img_num_per_cls(dataset, 100, 0.98)
    return gen_imbalanced_data(dataset, img_num_per_cls)
...
def get_cifar_100_datasets(train_transform, test_transform, train_classes=range(80),
                       prop_train_labels=0.8, split_train_val=False, seed=0, long_tailed=True):
    ...
    if long_tailed:
        test_dataset = subsample_test_dataset(deepcopy(test_dataset))
